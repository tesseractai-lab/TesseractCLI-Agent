...
class TestRoutingResolution:
    def test_get_llm_uses_main_model_when_no_task_given(self, mocker):
        ...
        fake_provider.get_model.return_value = "the-model"
        mocker.patch.object(dispatcher, "_get_provider", return_value=fake_provider)

        result = dispatcher.get_llm()

        fake_provider.get_model.assert_called_once_with(
            "llama3", temperature=0.3, max_tokens=4096
        )
    ...
    def test_get_llm_uses_task_specific_routing_when_present(self, mocker):
        ...
        dispatcher = LLMDispatcher(
            routing_table=_routing_table(
                main, default_routing={"code_generation": task_routing}
            )
        )
        ...
        fake_provider.get_model.assert_called_once_with(
            "claude-sonnet-5", temperature=0.1, max_tokens=4096
        )
...
@pytest.mark.asyncio          # ← class-level decorator
class TestFallbackChain:
    ...
    async def test_falls_back_when_primary_provider_unavailable(self, mocker):
        ...
        groq_provider.get_model_safe.return_value = None  # missing key, say
        ...
        def fake_get_provider(name):
            return {"groq": groq_provider, "cerebras": cerebras_provider}[name]

        mocker.patch.object(dispatcher, "_get_provider", side_effect=fake_get_provider)

        result = await dispatcher.ainvoke_with_fallback([HumanMessage(content="hey")])

        assert result == "from cerebras"
    ...
    async def test_falls_back_on_non_rate_limit_exception(self, mocker):
        ...
        assert result == "from cerebras"
        # non-rate-limit error -> no truncation retry attempted on groq
    ...
    async def test_truncates_and_retries_same_step_on_rate_limit_error(self, mocker):
        ...
        model.ainvoke = AsyncMock(
            side_effect=[Exception("429 rate limit exceeded"), success_result]
        )
        ...
        assert result == "truncated success"
        assert model.ainvoke.call_count == 2
        # second call should have gotten the truncated messages
...
class TestTruncateMessages:
    ...
    def test_only_last_message_truncated(self):
        ...
        result = LLMDispatcher._truncate_messages(messages, max_chars=100)
        assert result[0].content == "x" * 5000  # untouched
        assert len(result[1].content) < 5000  # truncated
