...
PROVIDER_CATALOG: tuple[ProviderSuggestion, ...] = (
    ProviderSuggestion(
        "groq", "Groq", ("llama-3.3-70b-versatile", "qwen/qwen3-32b", "gemma2-9b-it")
    ),
    ProviderSuggestion("cerebras", "Cerebras", ("llama3.1-8b", "llama-3.3-70b")),
    ProviderSuggestion(
        "mistral", "Mistral", ("mistral-small-latest", "open-mistral-nemo")
    ),
    ProviderSuggestion(
        "huggingface",
        "HuggingFace",
    ...
    ProviderSuggestion(
        "together", "Together AI", ("meta-llama/Llama-3.3-70B-Instruct-Turbo-Free",)
    ),
    ...
    ProviderSuggestion(
        "github_models", "GitHub Models", ("gpt-4o-mini", "Meta-Llama-3.1-8B-Instruct")
    ),
...
def suggestions_text() -> str:
    ...
    for entry in PROVIDER_CATALOG:
        ...
        lines.append(
            f"  [bold]{entry.provider:<14}[/bold] {entry.label} — e.g. {models}"
        )
    lines.append("")
    lines.append(
        "[dim]Suggestions only - confirm current model IDs/pricing with the provider before relying on them.[/dim]"
    )
    return "\n".join(lines)
