...
def _build_parser() -> argparse.ArgumentParser:
    ...
    settings.add_argument(
        "--list", action="store_true", help="list all packs and the models inside them"
    )
    settings.add_argument(
        "--suggest", action="store_true", help="show provider/model suggestions"
    )
    settings.add_argument(
        "--add",
        choices=["pool", "model"],
        help="add a new pool (pack) or a model inside one",
    )
    settings.add_argument(
        "--remove",
        choices=["pool", "model"],
        help="remove a pool (pack) or a model inside one",
    )
    settings.add_argument("--pack", help="pack name (required with --add/--remove)")
    settings.add_argument(
        "--provider", help="provider name (required with --add/--remove model)"
    )
    settings.add_argument(
        "--model", help="model identifier (required with --add/--remove model)"
    )
    settings.add_argument(
        "--fallback",
        action="store_true",
        help="target the fallback list instead of the primary pool",
    )
    settings.add_argument(
        "--set",
        nargs=2,
        metavar=("PATH", "VALUE"),
        help="set a dot-notation config value, e.g. paths.data_dir /tmp/data",
    )
    settings.add_argument(
        "--get",
        metavar="PATH",
        help="read a dot-notation config value, e.g. agent.temperature",
    )
...
def _run_settings(args: argparse.Namespace) -> None:
    ...
    try:
        if args.add == "pool":
            ...
            manager.packs.add_model(
                args.pack,
                args.provider,
                args.model,
                target="fallback" if args.fallback else "pool",
            )
            manager.save()
            console.print(
                f"[green]added {args.provider}/{args.model} to '{args.pack}'[/green]"
            )
            ...
            manager.save()
            console.print(
                f"[green]removed {args.provider}/{args.model} from '{args.pack}'[/green]"
            )
        elif args.set:
            path, value = args.set
